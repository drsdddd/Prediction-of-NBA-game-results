...
def clean_data(df):
    ...
    # 【新增优化】只保留常规赛数据，避免季前赛和季后赛的特殊策略导致数据分布偏移
    # API 返回的 SEASON_ID 中，以 '2' 开头的是常规赛（如 '22023'）
...
def create_matchup_dataset(df):
    ...
    base_cols = ['GAME_ID', 'GAME_DATE', 'TEAM_ID', 'TEAM_ABBREVIATION', 'SEASON_ID']
    ...
    # 【新增优化】根据 EDA 的热力图启发，强相关的特征往往是“差值”
    # 我们遍历所有的滚动特征，额外生成一列 主队 - 客队 的 DIFF 特征
    for col in feature_cols:
        col_name_home = f"{col}_HOME"
        col_name_away = f"{col}_AWAY"
        if col_name_home in matchup_df.columns and col_name_away in matchup_df.columns:
            diff_col_name = f"DIFF_{col}"
            matchup_df[diff_col_name] = matchup_df[col_name_home] - matchup_df[col_name_away]
...
def calculate_elo_features(matchup_df):
    """
    根据比赛结果和净胜分，按时间顺序动态计算每支球队的 ELO 等级分。
    包含机制：主场优势加成、跨赛季均值回归 (25%)、净胜分倍数 (MOV Multiplier)。
    """
    print("正在计算 ELO 等级分及跨赛季均值回归...")
    # 必须确保按时间顺序计算
    ...
    for idx, row in matchup_df.iterrows():
        ...
        season = row['SEASON_ID_HOME'] # 拿主队的 SEASON_ID 判断跨赛季
        
        # 1. 初始分数分配 (1500)
        if home_team not in elo_dict:
            elo_dict[home_team] = 1500.0
            last_season_dict[home_team] = season
        if away_team not in elo_dict:
            elo_dict[away_team] = 1500.0
            last_season_dict[away_team] = season
            
        # 2. 跨赛季均值回归 (软重置：向 1500 回归 25%)
        if last_season_dict[home_team] != season:
            elo_dict[home_team] = (elo_dict[home_team] * 0.75) + (1500 * 0.25)
            last_season_dict[home_team] = season
            
        if last_season_dict[away_team] != season:
            elo_dict[away_team] = (elo_dict[away_team] * 0.75) + (1500 * 0.25)
            last_season_dict[away_team] = season
            
        # 3. 获取赛前的 ELO（这才是我们要作为预测特征存入 DataFrame 的）
        home_elo = elo_dict[home_team]
        away_elo = elo_dict[away_team]
        home_elo_list.append(home_elo)
        away_elo_list.append(away_elo)
        
        # 4. 根据比赛结果更新 ELO (为了给下一场比赛用)
        win = row['TARGET_WIN_HOME']
        pts_diff = row['TARGET_PLUS_MINUS_HOME']
        
        # 给主队加 100 分虚拟主场优势，来计算预期胜率
        home_elo_adj = home_elo + 100.0
        expected_home = 1.0 / (1.0 + 10.0 ** ((away_elo - home_elo_adj) / 400.0))
        
        # 计算净胜分乘数 (Margin of Victory Multiplier)，避免大胜和小胜加分一样
        elo_diff = (home_elo_adj - away_elo) if win == 1 else (away_elo - home_elo_adj)
        mov_multiplier = ((abs(pts_diff) + 3) ** 0.8) / (7.5 + 0.006 * elo_diff)
        
        # ELO K 因子
        K = 20.0
        shift = K * mov_multiplier * (win - expected_home)
        
        # 真正写入字典
        elo_dict[home_team] = home_elo + shift
        elo_dict[away_team] = away_elo - shift

    matchup_df['ELO_HOME'] = home_elo_list
    matchup_df['ELO_AWAY'] = away_elo_list
    matchup_df['DIFF_ELO'] = matchup_df['ELO_HOME'] - matchup_df['ELO_AWAY']
    
    return matchup_df

def main():
    input_path = os.path.join("data", "raw", "raw_games_multi_seasons.csv")
    ...
    # 加入动态 ELO 特征
    matchup_df = calculate_elo_features(matchup_df)
    
    # 保存结果
    output_path = os.path.join(output_dir, "features_multi_seasons.csv")
